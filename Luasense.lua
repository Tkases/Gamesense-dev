...
local aa_fun = require "gamesense/antiaim_funcs"
...
local aa = {}
local function getbuild() return "dev" end
...
local inverter_ref = ui.reference("AA", "Anti-aimbot angles", "Inverter")
local yaw_ref, yaw_val = ui.reference("AA", "Anti-aimbot angles", "Yaw")

local last_switch = 0
local last_shot   = {}
local pending_switch = false
...
local desync_indicator = {
    ...
    colors = {
        left = {154, 226, 250},
        right = {255, 255, 255},
        left_alt = {109, 213, 250},
        right_alt = {253, 163, 180},
    }
}
local function render_desync_indicator()
    ...
    local offset = 0
    local fake_text = math.floor(100*aa_fun.get_desync())
    local center_x, center_y = screen_x / 2 + offset, screen_y / 2
    local get_desync = aa_fun.get_desync()
    for i = 1, 3 do
        desync_indicator.left[i] = render.lerp(desync_indicator.left[i], (fake_text > 32 and desync_indicator.colors.left[i]) or desync_indicator.colors.left_alt[i], 6)
        desync_indicator.right[i] = render.lerp(desync_indicator.right[i], (fake_text > 32 and desync_indicator.colors.right[i]) or desync_indicator.colors.right_alt[i], 6)
    end
    desync_indicator.fake_length = render.lerp(desync_indicator.fake_length, math.abs(get_desync), 6)
    renderer.gradient(center_x + 3 - 30, center_y + 42, desync_indicator.fake_length, 3, 
        desync_indicator.left[1], desync_indicator.left[2], desync_indicator.left[3], 255, 
        desync_indicator.right[1], desync_indicator.right[2], desync_indicator.right[3], 255, true)
    renderer.rectangle(center_x + 4 - 30, center_y + 42, 57, 3, 17, 17, 17, 190)
    renderer.gradient(center_x + 3 - 30, center_y + 42, desync_indicator.fake_length, 3, 
        desync_indicator.left[1], desync_indicator.left[2], desync_indicator.left[3], 255,
        desync_indicator.right[1], desync_indicator.right[2], desync_indicator.right[3], 255, true)  
    local text2 = render.gradient_text(
        desync_indicator.left[1], desync_indicator.left[2], desync_indicator.left[3], (fake_text < 32 and 255) or 230,
        desync_indicator.right[1], desync_indicator.right[2], desync_indicator.right[3], (fake_text < 32 and 255) or 150,
        "LAP                          FAKE") 
    renderer.text(center_x - 28, center_y + 45, 255, 255, 255, 255, "-", nil, text2)
end
...
return (function(tbl)
        ...
        ["visuals & misc"] = {
            ...
            ["visuals"] = {
                ...
                custom_indicator_text = ui.new_textbox("aa", "anti-aimbot angles", prefix("custom indicator text")),
                          logs = {
                enabled = ui.new_checkbox("aa", "anti-aimbot angles", prefix("Enable hit logs"), true),
                options = ui.new_multiselect("aa", "anti-aimbot angles", prefix("Log options"), 
                    {"Hit", "Miss", "Advanced", "Dormant hit", "Dormant miss", "Buy"}),  
                hit_color = ui.new_color_picker("aa", "anti-aimbot angles", prefix("Hit log color"), 182, 231, 23, 255),
                miss_color = ui.new_color_picker("aa", "anti-aimbot angles", prefix("Miss log color"), 231, 23, 23, 255),
                bracket_color = ui.new_color_picker("aa", "anti-aimbot angles", prefix("Bracket color"), 255, 255, 255, 255),
                buy_color = ui.new_color_picker("aa", "anti-aimbot angles", prefix("Buy log color"), 255, 255, 255, 255),             
                custom_format = ui.new_textbox("aa", "anti-aimbot angles", prefix("Custom log format"), ""),
                show_spread = ui.new_checkbox("aa", "anti-aimbot angles", prefix("Show spread angle"), true)
             } 
            },
            ["misc"] = {
                features = ui.new_multiselect("aa", "anti-aimbot angles", prefix("features"), {"fix hideshot", "animations", "legs spammer", "dt_os_recharge_fix", "gucci_killsay", "antibrute"}),
                spammer = ui.new_slider("aa", "anti-aimbot angles", "\nspammer", 1, 9, 1),
                autobuy = ui.new_combobox("aa", "anti-aimbot angles", prefix("auto buy"), {"off", "awp", "scout"}),
                desyncind = ui.new_combobox("aa", "anti-aimbot angles", prefix("desyncind"), {"off", "on"})
            }           
        },
        ["config"] = {
            config_selector = ui.new_combobox("aa", "anti-aimbot angles", prefix("select config"), {"aggressive delayed jitter", "safe delayed jitter", "aggressive jitter", "safe jitter", "perfect jitter", "perfect delay jitter", "Nextgen"}),
            load = ui.new_button("aa", "anti-aimbot angles", "\ac8c8c8FF load", function()
                ...
                local check, message = pcall(function()
                    local tbl
                    if selected_config == "Nextgen" then
                        tbl = json.parse([[{"LUASENSE":{"duck moving":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"extra":{"builder":{"builder":"moving","team":"t"},"submenu":"builder","features":{"backstab":"off","fixer":"default","legit":"off","roll":0,"safehead":{},"defensive":"off","distance":250,"fix":{},"states":{}},"keybinds":{"type_manual":"default","keys":{},"disablers":{},"type_freestand":"default","reset":false}},"slow motion":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"air duck":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"fake lag":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"air":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"global":{"t":{"nextgen":{"right":20,"left":-20,"tick":1,"jitter":25,"fake":25,"defensive":"always on"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"nextgen","luasense":{"right":0,"left":0,"fake":25,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":20,"method":"simple","left":-20,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":20,"left":-20,"tick":1,"jitter":25,"fake":25,"defensive":"always on"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"nextgen","luasense":{"right":0,"left":0,"fake":25,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":20,"method":"simple","left":-20,"defensive":"off","timer":150,"antibf":"no"}}},"duck":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"hide shot":{"t":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":0,"left":0,"tick":1,"jitter":0,"fake":0,"defensive":"off"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":0,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":0,"method":"simple","left":0,"defensive":"off","timer":150,"antibf":"no"}}},"moving":{"t":{"nextgen":{"right":20,"left":-20,"tick":1,"jitter":25,"fake":20,"defensive":"always on"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"nextgen","luasense":{"right":0,"left":0,"fake":20,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":20,"method":"simple","left":-20,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":20,"left":-20,"tick":1,"jitter":25,"fake":20,"defensive":"always on"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"nextgen","luasense":{"right":0,"left":0,"fake":20,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":20,"method":"simple","left":-20,"defensive":"off","timer":150,"antibf":"no"}}},"standing":{"t":{"nextgen":{"right":20,"left":-20,"tick":1,"jitter":0,"fake":30,"defensive":"always on"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":30,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":20,"method":"simple","left":-20,"defensive":"off","timer":150,"antibf":"no"}},"ct":{"nextgen":{"right":20,"left":-20,"tick":1,"jitter":0,"fake":30,"defensive":"always on"},"advanced":{"right":0,"left":0,"trigger":"a: brandon","defensive":"off"},"disabled":{},"type":"disabled","luasense":{"right":0,"left":0,"fake":30,"yaw":0,"defensive":"off","mode":{},"luasense":1},"normal":{"right":0,"jitter_slider":0,"yaw":0,"method":"default","left":0,"jitter":"off","custom_slider":60,"body":"off","defensive":"off","mode":{},"body_slider":0},"auto":{"right":20,"method":"simple","left":-20,"defensive":"off","timer":150,"antibf":"no"}}}}}]])
                        elseif selected_config == "aggressive delayed jitter" then 
    ...
    for i, v in next, tbl.states do
        ...
        for index, value in next, {"ct", "t"} do
            aa[v][value] = {
                ["type"] = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "type"), (v == "global" and {"normal", "luasense", "advanced", "auto","nextgen"} or {"disabled", "normal", "luasense", "advanced", "auto", "nextgen", "develop"})),
                ["normal"] = {
                ...
                ["develop"] = {
            yaw_mode = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "yaw mode\n"), {"static", "left/right", "tickbase delayed", "flick", "3 way", "5 way", "random l/r"}),
            yaw_left = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "yaw left\n"), -180, 180, -45),
            yaw_right = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "yaw right\n"), -180, 180, 45),
            yaw_delay = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "yaw delay\n"), 1, 10, 3),            
            jitter_mode = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "jitter mode\n"), {"off", "static", "left/right", "random", "3 way", "skitter"}),
            jitter_value = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "jitter value\n"), -180, 180, 30),          
            body_yaw = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "body yaw\n"), {"off", "static", "jitter", "opposite"}),
            body_yaw_value = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "body yaw value\n"), -180, 180, 60),
            defensive = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "defensive\n\n\n\n"), {"off", "luasense", "always on"}),
                },
                  ["luasense"] = {
...
elseif ui.get(menutbl["type"]) == "develop" then
    develop = develop or {
        three_way_status = 0,
        five_way_status = 0,
        current_tickcount = 0,
    ...
    function develop:manipulation_flick(min, max, delay)
        return client.random_int(min, max)
    end
    update = function(self, cmd)
        local current_tbl = menutbl[ui.get(menutbl["type"])]
        if not current_tbl then return end
        local state = "develop"
        local menu_ref = menu["anti aimbot"]["builder"][state]
        if not menu_ref then return end
        local lp = entity.get_local_player()
        if not lp or not entity.is_alive(lp) then return end
        self.three_way_status = globals.tickcount() % 3
        self.five_way_status = globals.tickcount() % 5
        local yaw_mode = ui.get(menu_ref["yaw_mode"])
        if yaw_mode == "tickbase delayed" then
            if globals.tickcount() > (self.current_tickcount + ui.get(menu_ref["yaw_delay"])) then
                if cmd.chokedcommands == 0 then
                    self.to_jitter = not self.to_jitter
                    self.inverter = self.to_jitter and 1 or -1
                    self.current_tickcount = globals.tickcount()
                end
            end
        elseif yaw_mode == "flick" then
            if self.manipulation_flick then
                self.inverter = self:manipulation_flick(-1, 1, ui.get(menu_ref["yaw_delay"]))
            else
                self.inverter = 1
            end
        else
            local pose = (entity.get_prop(lp, "m_flPoseParameter", 11) or 0)
            self.fake_body = math.max(-60, math.min(60, math.floor(pose * 120 - 60 + 0.5)))
            self.inverter  = self.fake_body > 0 and -1 or 1
        end

        local yaw_value = 0
        if yaw_mode == "static" then yaw_value = ui.get(menu_ref["yaw_static_slider"])
        elseif yaw_mode == "left/right"
            or yaw_mode == "tickbase delayed"
            or yaw_mode == "flick" then yaw_value = (self.inverter == -1) and ui.get(menu_ref["yaw_left"]) or ui.get(menu_ref["yaw_right"])
        elseif yaw_mode == "3 way" then
            local arr = { ui.get(menu_ref["yaw_1way"]), ui.get(menu_ref["yaw_2way"]), ui.get(menu_ref["yaw_3way"]) }
            yaw_value = arr[self.three_way_status + 1]
        elseif yaw_mode == "5 way" then
            local arr = {
                ui.get(menu_ref["yaw_1way"]), ui.get(menu_ref["yaw_2way"]), ui.get(menu_ref["yaw_3way"]),
                ui.get(menu_ref["yaw_4way"]), ui.get(menu_ref["yaw_5way"])
            }
            yaw_value = arr[self.five_way_status + 1]
        elseif yaw_mode == "random l/r" then
            local L, R = ui.get(menu_ref["yaw_left"]), ui.get(menu_ref["yaw_right"])
            if L > R then L, R = R, L end
            yaw_value = client.random_int(L, R)
        end

        local jitter_mode  = ui.get(menu_ref["jitter_mode"])
        local jitter_value = 0
        if     jitter_mode == "static" then jitter_value = ui.get(menu_ref["jitter_value"])
        elseif jitter_mode == "left/right" then jitter_value = (self.inverter == -1) and -ui.get(menu_ref["jitter_value"]) or ui.get(menu_ref["jitter_value"])
        elseif jitter_mode == "random" then
            local j = ui.get(menu_ref["jitter_value"])
            jitter_value = client.random_int(-j, j)
        elseif jitter_mode == "3 way" then
            local arr = { ui.get(menu_ref["jitter_1way"]), ui.get(menu_ref["jitter_2way"]), ui.get(menu_ref["jitter_3way"]) }
            jitter_value = arr[self.three_way_status + 1]
        elseif jitter_mode == "skitter" then
            local j = ui.get(menu_ref["jitter_value"])
            jitter_value = (globals.tickcount() % 2 == 0) and j or -j
        end

        local body_yaw_mode  = ui.get(menu_ref["body_yaw"])
        local body_yaw_value = 0
        if     body_yaw_mode == "static" then body_yaw_value = ui.get(menu_ref["body_yaw_value"])
        elseif body_yaw_mode == "jitter" then body_yaw_value = (globals.tickcount() % 2 == 0) and 60 or -60
        elseif body_yaw_mode == "opposite" then body_yaw_value = (self.inverter == 1) and -60 or 60
        end

        ui.set(tbl.items.yaw[1], "180")
        ui.set(tbl.items.yaw[2], yaw_value)
        ui.set(tbl.items.jitter[1], (jitter_mode ~= "off") and "offset" or "off")
        ui.set(tbl.items.jitter[2], jitter_value)
        ui.set(tbl.items.body[1], body_yaw_mode)
        ui.set(tbl.items.body[2], body_yaw_value)

        local defensive = ui.get(menu_ref["defensive"])
        if     defensive == "luasense" then
            cmd.force_defensive = (cmd.command_number % 3 ~= 1) or (cmd.weaponselect ~= 0) or (cmd.quick_stop == 1)
        elseif defensive == "always on" then
            ...
            elseif ui.get(menutbl["type"]) == "nextgen" then
                ...
                if (nextgen_.choke - nextgen_.last_packets >= nextgen_.delay) then
                    nextgen_.delay = ui.get(menutbl["tick"])
            ...
            local function apply_antibrute()
    if not pending_switch then return end
    pending_switch = false

    local mode = ui.get(menu["features"]["antibrute"].mode)
    if mode == "invert" then
        ui.set(inverter_ref, not ui.get(inverter_ref))
    elseif mode == "yaw add" then
        local val = ui.get(yaw_val)
        ui.set(yaw_val, (val + 30) % 180 - 90)
    end
end
            local function antibrute_switch()
    local tick = globals.tickcount()
    if tick - last_switch < ui.get(menu["anti-aimbot"]["features"]["antibrute"].delay) then return end
    last_switch = tick
    pending_switch = true
end

client.set_event_callback("player_hurt", function(e)
    if not ui.get(menu["anti-aimbot"]["features"]["antibrute"].enable) then return end
    local lp = entity.get_local_player()
    if client.userid_to_entindex(e.userid) ~= lp then return end
    local attacker = client.userid_to_entindex(e.attacker)
    if not attacker or attacker == lp or not entity.is_enemy(attacker) then return end
    antibrute_switch()
end)
client.set_event_callback("weapon_fire", function(e)
    if not ui.get(menu["anti-aimbot"]["features"]["antibrute"].enable) then return end
    local shooter = client.userid_to_entindex(e.userid)
    if shooter and entity.is_enemy(shooter) then
        last_shot[shooter] = globals.tickcount()
    end
end)
client.set_event_callback("bullet_impact", function(e)
    if not ui.get(menu["anti-aimbot"]["features"]["antibrute"].enable) then return end
    local shooter = client.userid_to_entindex(e.userid)
    if not shooter or not entity.is_enemy(shooter) then return end
    if last_shot[shooter] and globals.tickcount() - last_shot[shooter] < 32 then
        antibrute_switch()
        last_shot[shooter] = nil
    end
end)
        ...
        table.insert(tbl.callbacks["paint_ui"], function()
    if ui.get(menu["visuals & misc"]["misc"]["desync_indicator"]) then
            end,
        ["menu"] = function()
...
for i, v in next, menu do
    for index, value in next, v do
        ...
        ["arrows"] = function()
            ...
            if myself ~= nil and entity.is_alive(myself) then
                ...
                elseif text_to_draw == "luasense dev" then
    ...
    local function process_log_message(event_type, data)
    if not ui.get(menu["visuals & misc"]["visuals"].logs.enabled) then
        return
    end
    local message = ""
    local color = {255, 255, 255, 255}   
    if event_type == "hit" then
        color = {ui.get(menu["visuals & misc"]["visuals"].logs.hit_color)}
        message = string.format("[luasense] Hit %s's %s for %d damage (hp: %d)",
            data.target_name, data.hitgroup, data.damage, data.target_hp) 
        if ui.get(menu["visuals & misc"]["visuals"].logs.show_spread) and data.spread then
            message = message .. string.format(" (spread: %.1f°)", data.spread)
        end    
    elseif event_type == "miss" then
        color = {ui.get(menu["visuals & misc"]["visuals"].logs.miss_color)}
        message = string.format("[luasense] Missed %s's %s due to %s",
            data.target_name, data.hitgroup, data.reason)   
        if data.hit_chance then
            message = message .. string.format(" (hc: %d%%)", data.hit_chance)
        end       
    elseif event_type == "dormant_hit" then
        color = {ui.get(menu["visuals & misc"]["visuals"].logs.hit_color)}
        message = string.format("[dormant] Hit %s's %s for %d damage",
            data.target_name, data.hitgroup, data.damage)           
    elseif event_type == "dormant_miss" then
        color = {ui.get(menu["visuals & misc"]["visuals"].logs.miss_color)}
        message = string.format("[dormant] Missed %s's %s", 
            data.target_name, data.hitgroup)           
    elseif event_type == "buy" then
        color = {ui.get(menu["visuals & misc"]["visuals"].logs.buy_color)}
        message = string.format("%s bought %s", data.player_name, data.weapon)
    end            
    local r, g, b = unpack(color)
    notify.new_bottom(r, g, b, { { message } })
end
local log_handlers = {
    bullet_impact = function(e)
        local player = client.userid_to_entindex(e.userid)
        if player == entity.get_local_player() then
            log_data.impacts[#log_data.impacts + 1] = {
    ...
    aim_hit = function(e)
        if not ui.get(menu["visuals & misc"]["visuals"].logs.options):contains("Hit") then
            return
        end
        local shot = log_data.shots[e.id]
        if not shot then return end
        local target = entity.get(e.target)
        local spread = nil
        for i, impact in ipairs(log_data.impacts) do
            if impact.tick == globals.tickcount() then
                local eye_pos = {entity.get_origin(entity.get_local_player())}
                local shot_vec = {shot.x - eye_pos[1], shot.y - eye_pos[2], shot.z - eye_pos[3]}
                local impact_vec = {impact.x - eye_pos[1], impact.y - eye_pos[2], impact.z - eye_pos[3]}
                
                local shot_angles = {vector.angles(shot_vec)}
                local impact_angles = {vector.angles(impact_vec)}
                
                spread = math.sqrt(
                    (shot_angles[1] - impact_angles[1])^2 + 
                    (shot_angles[2] - impact_angles[2])^2
                )
                break
            end
        end     
        process_log_message("hit", {
    ...
    aim_miss = function(e)
        if not ui.get(menu["visuals & misc"]["visuals"].logs.options):contains("Miss") then
            return
        end        
        local shot = log_data.shots[e.id]
        if not shot then return end
        process_log_message("miss", {
            target_name = entity.get_player_name(e.target),
            hitgroup = hitgroup_names[e.hitgroup + 1] or "?",
            reason = e.reason,
    ...
    player_death = function(e)
        ...
        for id, shot in pairs(log_data.shots) do
            if current_tick - shot.tick > 128 then
                log_data.shots[id] = nil
            end
        end
        for i = #log_data.impacts, 1, -1 do
            if current_tick - log_data.impacts[i].tick > 128 then
                table.remove(log_data.impacts, i)
            end
        end
    end,
    item_purchase = function(e)
        if not ui.get(menu["visuals & misc"]["visuals"].logs.options):contains("Buy") then
            return
        end
        local player = client.userid_to_entindex(e.userid)
        process_log_message("buy", {
            player_name = entity.get_player_name(player),
            weapon = e.weapon
        })
    end
}
    ...
    bullet_impact = client.get_event_callback("bullet_impact"),
    aim_fire = client.get_event_callback("aim_fire"),
    aim_hit = client.get_event_callback("aim_hit"),
    aim_miss = client.get_event_callback("aim_miss"),
    player_death = client.get_event_callback("player_death"),
    item_purchase = client.get_event_callback("item_purchase")
}
client.set_event_callback("bullet_impact", function(e)
    if original_callbacks.bullet_impact then original_callbacks.bullet_impact(e) end
    log_handlers.bullet_impact(e)
end)
client.set_event_callback("aim_fire", function(e)
    if original_callbacks.aim_fire then original_callbacks.aim_fire(e) end
    log_handlers.aim_fire(e)
end)
client.set_event_callback("aim_hit", function(e)
    if original_callbacks.aim_hit then original_callbacks.aim_hit(e) end
    log_handlers.aim_hit(e)
end)
client.set_event_callback("aim_miss", function(e)
    if original_callbacks.aim_miss then original_callbacks.aim_miss(e) end
    log_handlers.aim_miss(e)
end)
client.set_event_callback("player_death", function(e)
    if original_callbacks.player_death then original_callbacks.player_death(e) end
    log_handlers.player_death(e)
end)
client.set_event_callback("item_purchase", function(e)
    if original_callbacks.item_purchase then original_callbacks.item_purchase(e) end
    log_handlers.item_purchase(e)
end)
